...
import org.javawebstack.http.router.router.annotation.params.*;
import org.javawebstack.http.router.router.annotation.verbs.*;
...
public class RouteBinder {
    ...
    public RouteBinder(HTTPRouter router) {
        this.router = router;
    }
    ...
    public void bind(String globalPrefix, Object controller) {
        ...
        for (Method method : getMethodsRecursive(controller.getClass())) {
            ...
            for (WebSocketMessage a : getAnnotations(WebSocketMessage.class, method)) {
                ...
                if (handler == null) {
                    ...
                    for (String prefix : prefixes)
                        router.webSocket(buildPattern(globalPrefix, prefix, a.value()), handler);
                    websocketHandlers.put(a.name(), handler);
                }
                handler.messageHandler = new BindMapper(router, controller, method);
            }
            for (WebSocketConnect a : getAnnotations(WebSocketConnect.class, method)) {
                ...
                handler.connectHandler = new BindMapper(router, controller, method);
            }
            for (WebSocketClose a : getAnnotations(WebSocketClose.class, method)) {
                ...
                handler.closeHandler = new BindMapper(router, controller, method);
            }
            //endregion

            if (binds.size() > 0) {
                BindHandler handler = new BindHandler(router, controller, method);
                for (String prefix : prefixes) {
                    for (Bind bind : binds) {
                        router.route(bind.method, buildPattern(globalPrefix, prefix, bind.path), handler);
                    }
                }
            }
        }
    ...
    private void bindMiddlewares(HTTPMethod method, String globalPrefix, List<String> prefixes, String path, List<String> middlewares) {
        for (String name : middlewares) {
            RequestHandler before = router.getBeforeMiddleware(name);
            AfterRequestHandler after = router.getAfterMiddleware(name);
            for (String prefix : prefixes) {
                if (before == null && after == null) {
                    router.getLogger().warning("Middleware \"" + name + "\" not found!");
                    continue;
                }
                if (before != null)
                    router.beforeRoute(method, buildPattern(globalPrefix, prefix, path), before);
                if (after != null)
                    router.afterRoute(method, buildPattern(globalPrefix, prefix, path), after);
            }
        }
    }
    ...
    private static class BindMapper {
        ...
        public BindMapper(HTTPRouter router, Object controller, Method method) {
            this.router = router;
        ...
        public Object invoke(Exchange exchange, Map<String, Object> extraArgs) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (a instanceof Body) {
                    ...
                    for (RouteAutoInjector autoInjector : router.getRouteAutoInjectors()) {
    ...
    private static class BindHandler implements RequestHandler {
        ...
        public BindHandler(HTTPRouter router, Object controller, Method method) {
            handler = new BindMapper(router, controller, method);
        }
